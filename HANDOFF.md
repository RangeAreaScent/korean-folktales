...
### 7.4b Google AdSense (v1.1 착수 → 1차 반려 → 스코핑 fix → 2차 반려 → 홈페이지 콘텐츠 보강, 재심사 대기)
- Publisher ID: `ca-pub-2376980284402579`
- `public/ads.txt`: `google.com, pub-2376980284402579, DIRECT, f08c47fec0942fa0`
- **1차 반려**: "Google-served ads on screens without publisher-content ... used for alerts, navigation or other behavioral purposes" + "Low value content". 원인은 AdSense 로더 스크립트가 루트 레이아웃에 있어 `/`(피커+색칠 캔버스+플립북 앱 전체)에도 로드되던 것
- **스코핑 fix** (commit `adab522` + 누락분 보완 `3ef16fd`): `about`/`privacy`/`folktales/[slug]`를 `src/app/(content)/` route group(URL 안 바뀜)으로 이동 + 스크립트 전용 `layout.tsx` 신설, 루트 레이아웃에서 제거. `.next/server/app/*.html` grep으로 `/`엔 없고 콘텐츠 페이지엔 있는 것 확인 (§10.36)
  - ⚠️ **교훈**: `adab522` 커밋 시 `git status`의 앞 컬럼 공백(unstaged) vs 문자(staged) 구분을 놓쳐서 `layout.tsx` 스크립트 제거가 실제로는 커밋 안 됐던 적이 있음 — 커밋 전 항상 `git status --short`로 스테이징 상태 문자 확인할 것
- **2차 반려 — 스코핑 fix 배포 9일 후에도 동일 사유** (AdSense Sites 페이지, Last updated 2026-07-22): 스크립트가 `/`에 이미 없는데도 계속 반려됨 → **원인이 스크립트 위치가 아니라 `/`(홈페이지) 자체의 콘텐츠 깊이**라고 재진단. `/`의 SSR HTML을 직접 curl로 뜯어보니 실제 텍스트는 히어로 카피 + 8개 스토리 티저 2줄뿐(~1800자) — 진짜 읽을거리(원작 유래·요약·글로서리)는 클릭해야 열리는 모달/서브페이지 안에 있어서, 홈페이지 자체가 "콘텐츠"보다 "이야기 고르는 메뉴/네비게이션 화면"으로 읽힘. Google이 지적한 "used for... navigation... purposes"가 정확히 이 구조를 가리킨 것으로 추정
- **콘텐츠 보강 fix**: `StoryPicker.tsx`의 스토리 카드 그리드 **아래**(피커 화면 최하단, 기존 카드/그리드는 손대지 않음)에 "Origins" 섹션 신규 추가 — 8개 스토리 각각의 `story.originalTale.origin`(이미 `/folktales/[slug]`·모달에 쓰이던 실제 유래 텍스트)을 재사용해 서버 렌더로 항상 노출(클릭 불필요) + `/folktales/{slug}` 링크. 새 글 작성 없이 기존 콘텐츠 노출 위치만 확장. UI 문자열 4개(`originsEyebrow/Title/Intro/ReadMore`)만 `strings.ts`에 신규 추가
  - **모바일/iOS 레이아웃 영향 없음** — StoryPicker는 색칠 캔버스(모바일 하단시트·팔레트·iOS WebView)와 완전히 분리된 별개 컴포넌트라, 피커 화면 최하단에 섹션을 추가해도 v1.2에서 공들인 캔버스/팔레트 레이아웃과 전혀 안 겹침. 실제로 모바일 뷰포트에서 스토리 진입 후 NarrationModal/MobileToolbar/MobilePaletteSheet 정상 동작 확인함
- 재심사는 이 fix 배포 후 사용자가 AdSense 콘솔에서 직접 요청 (배포 직후 바로 요청하지 말고, 크롤러 재방문 시간을 두는 게 안전 — 2차 반려가 이 타이밍 실수였을 가능성 있음)
...
- **1차 반려** ❌ → **스코핑 fix** (commit `adab522`+`3ef16fd`) → **2차 반려 (동일 사유, fix 배포 9일 후에도)** ❌ → **홈페이지 콘텐츠 보강 fix 완료, 재심사 대기** ⏳ — 스크립트 위치 문제가 아니라 `/`(홈페이지) 자체 콘텐츠 깊이 부족이 근본 원인으로 재진단, StoryPicker 하단에 8개 스토리 origin 콘텐츠 섹션 추가 (§7.4b)
- **재심사 결과 대기** ⏳ — 배포 후 바로 재요청하지 말고 며칠 두고 요청할 것(§7.4b 타이밍 교훈). 통과 후 남은 일: 실제 `<ins class="adsbygoogle">` 광고 유닛 배치 (현재 스크립트만 있고 유닛은 0개), 배치 위치 논의 (about/privacy/folktale 페이지 내 위치)
...
### 최신 작업 (v1.2 후속 — AdSense 반려 대응, 2 라운드)

**1라운드 — AdSense 1차 심사 반려 → 스코핑 fix** (commits `adab522`, `3ef16fd`):
...
- ⚠️ 첫 커밋(`adab522`)에서 `git status --short`의 공백(unstaged) vs 문자(staged) 표시를 놓쳐서 `layout.tsx`의 스크립트 제거가 실제로는 커밋 안 됐던 실수 발견 → `3ef16fd`로 뒤늦게 수정. **교훈: 커밋 전 반드시 `git status --short`로 스테이징 여부 재확인**
- `npx next build` 후 `.next/server/app/*.html`에서 `grep adsbygoogle`로 `/`엔 없고 콘텐츠 페이지엔 있는 것 확인
- `public/coloring/` 아래 도안 PNG 변경분(별도 진행 중이던 무관 작업)은 커밋에서 제외, 스테이징하지 않음
- 사용자 질문으로 트레이드오프 논의: 콘텐츠 있는 피커 화면도 `/`에 얹혀 있어 스코프에서 함께 빠짐 — AdSense Auto Ads는 URL 단위로만 걸리기 때문에 SPA 상태별 분리가 불가능함이 핵심 (§10.36)

**2라운드 — 동일 사유로 2차 반려 → 근본 원인 재진단 → 홈페이지 콘텐츠 보강**:
- fix 배포 9일 후에도 AdSense Sites 페이지에 동일 사유("Needs attention")로 남아있는 것 확인 (Last updated 2026-07-22) — 스크립트가 이미 `/`에 없는데도 반복 반려되는 건 스코핑 문제가 아니라 **`/` 자체의 콘텐츠 깊이 부족**이 원인이라고 재진단
- `/` SSR HTML을 curl로 직접 분석 — 실제 텍스트는 히어로 카피 + 스토리 티저 2줄뿐(~1800자), 진짜 내용(원작 유래·글로서리)은 클릭해야 열리는 모달 안에 있어 크롤러/리뷰어에게는 "콘텐츠"보다 "메뉴 화면"으로 읽힘
- 사용자가 모바일/iOS 레이아웃이 꼬일까 우려 → StoryPicker가 색칠 캔버스(모바일 하단시트·iOS WebView)와 완전히 분리된 컴포넌트임을 확인시키고, 피커 화면 **최하단**(기존 카드 그리드는 그대로 두고) 에 새 섹션만 추가하는 방식으로 리스크 없이 진행
- `StoryPicker.tsx`에 "Origins" 섹션 신규 추가 — 8개 스토리의 `story.originalTale.origin`(이미 존재하던 실제 유래 텍스트, 새로 안 씀)을 서버 렌더로 항상 노출 + `/folktales/{slug}` 링크. `strings.ts`에 UI 문자열 4개만 추가
- 검증: `npx tsc --noEmit` + `eslint` + `next build` 통과, 빌드된 정적 HTML에서 8개 링크 전부 확인. dev 서버에서 데스크탑/모바일(375px) 접근성 트리로 레이아웃 확인 + 실제 스토리 진입해 모바일 NarrationModal/MobileToolbar/MobilePaletteSheet 정상 동작 재확인 (참고: 이 세션 Browser pane 스크린샷이 계속 검은 화면만 반환하는 환경 이슈 있었음 — get_page_text/read_page로 대체 검증. v1.1에서도 겪은 동일 이슈)
- 아직 커밋 전 (사용자 확인 대기)

**다음 세션 체크**: 이번 콘텐츠 보강 fix 커밋·배포 여부 확인. 배포 후 며칠 두고 재심사 요청 → 결과 확인. 통과 시 실제 광고 유닛 배치 위치 논의 + 피커 라우트 분리로 노출 확대할지 결정.
...
*마지막 갱신: v1.2 후속 · AdSense 반려 대응 2라운드 — 1차(스크립트 스코핑) fix 후에도 동일 사유로 2차 반려, 근본 원인을 홈페이지(`/`) 콘텐츠 깊이 부족으로 재진단해 StoryPicker 하단에 8개 스토리 origin 콘텐츠 섹션 추가(모바일/iOS 캔버스 레이아웃과 무관). 커밋 대기 중. 다음: 재심사 결과 확인 + iOS 앱 출시(Apple Developer 가입 전제).*
