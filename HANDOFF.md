...
## 2. 현재 버전 · v1.1 — 사운드/배경 무드 + 수익화 인프라
...
| **데스크탑 레이아웃 (v1.1 재조정)** | max-w **1700px** · canvas max-w **768px** (v1.0의 960px에서 20% 축소 — 랩탑 한 화면에 캔버스+팔레트 다 들어오도록) · sidebar **360px** · canvas image **rounded border 제거 → shadow only** · max zoom **8x** |
| **DesktopControlBar** | 캔버스 바로 아래 inline strip — `[↶ Undo][🗑 Erase]    [⊖ 100% ⊕]    [⛶ Fullscreen]`. 100% 버튼 tooltip에 트랙패드 핀치 안내 (v1.1) |
| **MobileControlBar** | `[↶][1/N][🗑]    [📖][🔊][🎵][Continue/Choose →]` — undo·페이지 인디케이터 분리, primary action 우측 끝 (엄지 위치), locale toggle은 NarrationModal로 이동. v1.1: BgmToggle 추가 |
...
| **배경 톤 (v1.1: 회전형으로 전면 교체)** | v1.0의 고정 Cool minimal white가 "차갑고 회색적, 설화 느낌 안 남" 피드백 → **세션당 무작위 회전 6색** (단청 주홍·등불 자주·청자 세이지·홍시 감·자수정 플럼·쪽빛 하늘, 오방색 스펙트럼). `sessionStorage`로 방문 내내 유지, 새 세션마다 재추첨. **액센트 색(eyebrow/pill/divider)도 배경과 함께 회전** — 팔레트별 전용 accent hex |
...
│   ├── ColoringCanvas.tsx          캔버스 — 줌/팬/핀치(midpoint anchor)/풀스크린/erase mode. `hideToolbar` 항상 true. v1.1: 트랙패드 핀치(ctrl+wheel) 지원, max-w 768px
...
│   ├── MobileControlBar.tsx        모바일 캔버스 아래 control strip (undo + page indicator + erase + 📖 story + 🔊 + 🎵 BGM + primary action)
...
│   └── BgmToggle.tsx               ⭐ v1.1 신규 — 배경음악 on/off (음표 아이콘, SoundToggle과 동일 스타일)

public/
├── sounds/                         ⭐ v1.1 신규 — paint1/2/3.m4a, page-turn.mp3, chime.m4a, bgm.m4a (전부 압축됨, 원본은 로컬 /sound/ 에 gitignore)
├── ads.txt                         ⭐ v1.1 신규 — Google AdSense publisher 인증 라인
└── coloring/
...
macOS Safari/Chrome은 트랙패드 두 손가락 핀치를 **`wheel` 이벤트 + `ctrlKey: true`** 로 브라우저에 전달한다 (사용자가 실제 Ctrl을 누르지 않아도). `ColoringCanvas.tsx`의 `handleWheel`이 이를 감지해 `e.preventDefault()` (네이티브 페이지 줌 차단) 후 커서 위치를 anchor로 확대/축소 — 터치 핀치(§4.15)와 동일한 `(scroll + local) * ratio - local` 공식. `wheelZoomRef`로 rAF 코얼레싱해 빠른 휠 이벤트 연속에도 부드럽게 처리.
...
### 7.4b Google AdSense (v1.1 착수 — 심사 대기)
- Publisher ID: `ca-pub-2376980284402579`
- `layout.tsx`의 `<body>` 최상단에 **raw JSX `<script>`** 태그 (⚠️ `next/script` 아님 — §10.26 참고)
- `public/ads.txt`: `google.com, pub-2376980284402579, DIRECT, f08c47fec0942fa0`
- 심사 결과는 이메일로 옴. 승인 전엔 광고 슬롯이 비어있거나 안 보이는 게 정상
...
- **심사 결과 대기** ⏳ — 보통 며칠~2-3주. 승인되면 광고 슬롯 배치 위치 논의 필요 (스토리 사이 vs 갤러리 하단 등)
...
### 마지막 작업 (v1.1, commits fab7fd4..c265068 — 15개)
...
### 현재 진행 트랙
없음 (v1.1 사운드/배경/수익화 트랙 완료). AdSense 심사 결과 대기 중 (§7.4b). 다음 자연스러운 후보:
1. **AdSense 승인 대기** — 결과 오면 광고 슬롯 위치 결정 + consent 배너 설정
2. **Pinterest 첫 푸시** — 도메인 인증 완료, 8 V2.5 production이라 한 번에 30 핀 가능 (PINTEREST_PLAYBOOK.md §10)
3. **Touch tap target 44pt** (`@media (pointer: coarse)`)
4. **Smart palette per scene** — `Scene.suggestedColors?: Hsl[]` (각 도안에 어울리는 6-8색 큐레이션)
5. **즐겨찾는 색** 별 토글
6. **a11y 감사**
7. **GA / Plausible 추가** + GSC Request Indexing

### 사용자 톤 / 워크스타일
- 빠른 결정, 깔끔한 시각, 디테일 신경, 안정성 우선
- 응답 패턴: "응 가자" / "잘된다" / "이거 해줘" — 명확한 짧은 신호
- 한국어로 토론 + 영어 검색
- **Root cause 지적 선호** — surface 증상만 보지 말고 (모바일 클릭 안 됨 → viewport meta? button overlay? 결국 next.config.ts의 `allowedDevOrigins` 였음. v1.1에서도 반복: AdSense verify 실패가 처음엔 계정/DNS 문제로 보였지만 실제론 next/script의 HTML 렌더링 방식 문제였음 — §10.26)
- 시각 디자인 iteration 즐김 — v1.0 배경 톤만 9번 바꿈, v1.1에서는 "정답 찾기"를 포기하고 회전형으로 컨셉 전환 (다양성 자체를 기능화)
- **스크린샷보다 Artifact 선호** — 이 세션에서 preview_screenshot 결과가 사용자 화면에 안 보이는 이슈 발견. 색상/시각 비교가 필요하면 Artifact(HTML)로 만들어 보여줄 것

---

*마지막 갱신: v1.1 · 사운드 실제 샘플 교체 + BGM 추가, 배경/액센트 세션당 회전(오방색 6색), 데스크탑 캔버스 20% 축소 + 트랙패드 핀치줌, 픽커 히어로 카피 재작성, Bing/AdSense/Pinterest 검증. 다음 트랙: AdSense 심사 대기 → 결과 후 광고 배치 또는 Pinterest 첫 푸시.*
