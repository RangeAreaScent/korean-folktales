...
### 마지막 작업 (v1.2, commits a19b50b..eabacb0 — 7개)
...
*마지막 갱신: v1.2 · Capacitor iOS 앱 스캐폴딩(시뮬레이터 검증) + 모바일 팔레트 iOS 재설계(peek/expand 바텀시트·segmented·blur) + 툴바를 팔레트 위 별도 줄 + 하단 고정 시트 + 모바일 캔버스 세로 확대(화면 전체 사용) + 회전 배경 전 페이지 확장 + 페이지 인디케이터 헤더 이동 + lint 클린업. 다음: iOS 앱 출시(Apple Developer 가입 전제).*
